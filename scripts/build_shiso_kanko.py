...
def スポット(h, url):
    ...
    # ★電話は番号だけに正す（2026-08-23の点検で発覚：全角ハイフンの
    #   「0790‐62‐9580」、時間の混ざった「080-… （8：30～17：00)」、
    #   番号でない「ー」がそのまま入り、読み上げが壊れる）。
    #   添え書き（受付時間等）は捨てずに「電話補足」へ残す
    if "電話" in it:
        生 = it["電話"].replace("‐", "-").replace("－", "-").replace("−", "-")
        m = re.search(r"0\d{1,4}-\d{1,4}-\d{3,4}", 生)
        if m:
            it["電話"] = m.group(0)
            補足 = (生[:m.start()] + 生[m.end():]).strip(" 　・、")
            if 補足:
                it["電話補足"] = 補足[:60]
        else:
            del it["電話"]       # 番号が無い（「ー」等）＝電話欄を出さない
