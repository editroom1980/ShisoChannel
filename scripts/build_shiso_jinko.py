...
def 主():
    ...
    出["年ごと"] = 年ごと
    # ★1世帯あたりの人数（2026-08-28）。人口は3割減ったのに世帯数は増えている。
    #   1世帯 3.32人 → 2.24人。ひとり暮らし・ふたり暮らしが増えたということ。
    #   見守りや配食の必要性を語るときの土台になる数字なので、まとめに置く
    出["まとめ"]["世帯"] = {
        "最古の月": 古, "最古の世帯数": 全[古]["宍粟市"]["世帯"],
        "最古の1世帯あたり": round(全[古]["宍粟市"]["計"] / 全[古]["宍粟市"]["世帯"], 2),
        "最新の月": 新, "最新の世帯数": 全[新]["宍粟市"]["世帯"],
        "最新の1世帯あたり": round(全[新]["宍粟市"]["計"] / 全[新]["宍粟市"]["世帯"], 2),
        "世帯の増減": 全[新]["宍粟市"]["世帯"] - 全[古]["宍粟市"]["世帯"],
    }
    if 年ごと:
        年順 = sorted(年ごと)
        出["まとめ"]["出生"] = {
    ...
    if "世帯" in m:
        w = m["世帯"]
        print(f"\n世帯 {w['最古の世帯数']:,} → {w['最新の世帯数']:,}（{w['世帯の増減']:+,}）"
              f"／1世帯あたり {w['最古の1世帯あたり']}人 → {w['最新の1世帯あたり']}人")
    if "出生" in m:
        b = m["出生"]
