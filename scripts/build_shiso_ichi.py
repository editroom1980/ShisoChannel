...
def 施設名に直す(題):
    """記事の題から、施設の名前だけを取り出す（2026-08-28）。
       ★実測で混ざっていたもの：
         「にしはりまクリーンセンターへごみの持込み」→「にしはりまクリーンセンター」
         「千種保健福祉課の紹介」→「千種保健福祉課」
       施設名を言われたときに当てるのが目的なので、尾ひれを落とす"""
    t = 題.strip()
    t = re.sub(r"(へ|に)(の)?(ごみの)?(持込み|持ち込み|お越しの方へ).*$", "", t)
    t = re.sub(r"の(紹介|ご案内|案内|概要|使い方|利用案内)$", "", t)
    t = re.sub(r"について$", "", t)
    # 括弧つきの別名は括弧を「」に揃える（同じ施設が2件に割れるのを防ぐ）
    t = t.replace("（", "「").replace("）", "」")
    return t.strip()
...
def 座標を拾う(生html):
    ...
    for 型 in (r"maps\.google\.co\.jp/maps\?q=(-?\d+\.\d+),(-?\d+\.\d+)",
        ...
        if m:
            ...
            # ★宍粟市の実際の広がり（2026-08-28に実測して狭めた）。
            #   住所の取れている60施設は 緯度34.968〜35.214／経度134.418〜134.651 に収まる。
            #   前の範囲（34.8〜35.4）は市外まで含んでいて、
            #   たつの市の施設の座標を「宍粟市の施設」として取り込んでいた
            #   （「飼い猫の保護」10.7km・「にしはりまクリーンセンター」9.8km）。
            #   実測の外側に少し余裕を持たせて 34.94〜35.25／134.38〜134.68 とする
            if 34.94 <= y <= 35.25 and 134.38 <= x <= 134.68:
                return y, x
    return None
